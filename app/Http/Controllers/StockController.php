...
class StockController extends Controller
{
   ...
   public function SendItem(Request $request){
    ...
    foreach($tableData as $td){
     ...
     // update stock
     $update_stock_total = DB::table('dsr_stocks')->where('id','=',$dsr_stock->id)->increment('total', $td["qty"]); 
...
public function GetStockItemsById(Request $request){
    ...
    if($stock_id == 0){
        ...
        print_r($all_dsr_items);
        exit();
        // $object = new stdClass();
        // for ($i=0; $i < count($all_dsr_items); $i++) { 
        //     $object->name = $all_dsr_items[$i]->name;
        //     $object->qty = $all_dsr_items[$i]->qty;
        // }
        // $allData[] = $object;

        return view('admin.item.view_balance', ['dsrList'=>$dsrs,'dsrStockData'=>$allData]);
