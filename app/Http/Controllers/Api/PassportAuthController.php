...
class PassportAuthController extends Controller
{
    ...
    public function MobileDsrStockDataItem($results)
    {
        ...
        for ($x = 0; $x < count($results); $x++) {
            $allData[] = (object) ['bulk_id' => $results{
                $x}->id, 'bulk_created_at' => $results{
                    $x}->created_at, 'items' => $itemData[$x]];
                }
            ...
            public function MobileGetDsrStockIds(Request $request)
            {
                ...
                for ($x = 0; $x < count($stock_data); $x++) {
                    if (isset($itemData[$x][0]->qty)) {
                        $allData[] = (object) ['stock_id' => $stock_data{
                            $x}->id, 'qty' => $itemData[$x][0]->qty];
                        }
                    }
                ...
                public function MobileGetItemCount(Request $request)
                {
                    ...
                    for ($x = 0; $x < count($stock_item_data); $x++) {
                        $allData[] = (object) ['item_id' => $stock_item_data{
                            $x}->item_id, "qty_sum" => $stock_qty_sum[$x]->qty_sum, "return_qty" => $srqs[$x][0]->return_qty];
                        }
            ...
            // update dsr stock (+)
                    ...
                    public function MobileDsrBankings(Request $request)
                    {
                        ...
                        if (count($bsum) == 0) {
                            ...
                            DB::insert('insert into pending_sum (dsr_id, date,banking_sum,banking_sampath,banking_cargils,banking_peoples,banking_sampth_online) values (?,?,?,?,?,?,?)', array($dsrId, $todayDate, $bankingSum, $sampath, $cargils, $peoples, $sampathonline));
                            $sum_id = DB::table('pending_sum')->latest('id')->first();
                        } else {
                            ...
                            DB::update('update pending_sum set banking_sum = banking_sum + ? ,banking_sampath = banking_sampath+ ? ,banking_cargils = banking_cargils+ ? ,banking_peoples = banking_peoples + ?, banking_sampth_online = banking_sampth_online + ?  where dsr_id = ? and date = ?', array($bankingSum, $sampath, $cargils, $peoples, $sampathonline, $dsrId, $todayDate));
                            foreach ($bsum as $sum) {
                                $sum_id = $sum;
                            }
                        }
        ...
        // check if there is data in pending sum table for dsr today
            ...
            // insert
                            foreach ($dbankingItems as $dbank) {
                                ...
                                $single_bank = DB::table('banks')->whereIN('id', [$dbank['bank_id']])->where('status', 1)->get();
                            ...
                            DB::insert('insert into pending_sum (dsr_id, date, direct_banking_sum, direct_banking_sampath, direct_banking_cargils, direct_banking_peoples, direct_banking_sampth_online) values (?,?,?,?,?,?,?)', array($dsrId, $todayDate, $dbankingSum, $sampath, $cargils, $peoples, $sampathonline));
                            $sum_id = DB::table('pending_sum')->latest('id')->first();
                        } else {
            ...
            // update
                            ...
                            DB::update(
                                ...
                                where dsr_id = ?
                                and date = ?',
                                array($dbankingSum, $sampath, $cargils, $peoples, $sampathonline, $dsrId, $todayDate)
                            );
        ...
        // check if there is data in pending sum table for dsr today
            ...
            // update cheque amount in dsrs table
                            ...
                            $cheques_is_exist = DrsCheque::where('sum_id',$sum_id->id)->get();
                            ...
                            if(count($cheques_is_exist) !=0 ){
                                ...
                                if(count($chequeArr) !=0){
                                    ...
                                    $cheque_list = DrsCheque::where('sum_id',$sum_id->id)->get();
                            ...
                            if($request->get('cash') == "" || $request->get('cash') == 0){

                               DB::update('update pending_sum set inhand_cheque = ? WHERE id =?', array($cheque_amount, $sum_id->id));
                               DB::update('update dsrs set cheque = ? WHERE sum_id =?', array($cheque_amount, $sum_id->id));

                               $dsrs_table_data = Dsr::where('sum_id',$sum_id->id)->get();

                                foreach($dsrs_table_data as $dsr_data){

                                    print_r("sdasd".$dsr_data->cash);

                                     DB::update('update dsrs set in_hand = ?, cash = ? WHERE id =?', array(($dsr_data->cash+$cheque_amount), $dsr_data->cash ,$sum_id->id));

                                    DB::update('update pending_sum set inhand_sum = ?,inhand_cash = ? WHERE id =?', array(($dsr_data->cash+$cheque_amount), $dsr_data->cash ,$sum_id->id));
                                }


                           }else{
                               DB::update('update pending_sum set inhand_sum = ?, inhand_cash = ?, inhand_cheque = ? WHERE id =?', array(($request->get('cash')+$cheque_amount), $request->get('cash'), $cheque_amount, $sum_id->id));

                               DB::update('update dsrs set in_hand = ?, cash = ?, cheque = ? WHERE sum_id =?', array(($request->get('cash')+$cheque_amount), $request->get('cash'), $cheque_amount, $sum_id->id));
                           }
                    ...
                    for ($x = 0; $x < count($pdsr); $x++) {
                        $allData[] = (object) ['id' => $pdsr{
                            $x}->id, 'dsr_id' => $pdsr{
                                $x}->dsr_id, 'name' => $pdsr{
                                    $x}->name, 'date' => $pdsr{
                                        $x}->date, 'inhand_sum' => $pdsr{
                                            $x}->inhand_sum, 'sales_sum' => $pdsr{
                                                $x}->sales_sum, 'credit_sum' => $pdsr{
                                                    $x}->credit_sum, 'credit_collection_sum' => $pdsr{
                                                        $x}->credit_collection_sum, 'banking_sum' => $pdsr{
                                                            $x}->banking_sum, 'direct_banking_sum' => $pdsr{
                                                                $x}->direct_banking_sum, 'retialer_item_count' => $reData[0]->retialer_item_count];
                                                            }
        ...
        //   ->get();
            ...
            // Oops.. Error Occured!
                                                        ...
                                                        public function MobileGetBankingSumery(Request $request)
                                                        {
                                                            ...
                                                            ->select('bankings.id', 'banks.bank_name', 'banks.id as bank_id', 'bank_ref_no', 'bank_amount', 'sum_id', 'bankings.dsr_id')
                                                            ->where('pending_sum.date', '=', $request->get('date'))
                                                            ->where('pending_sum.dsr_id', '=', $request->get('dsr_id'))
            ...
            // Oops.. Error Occured!
                                                        ...
                                                        public function MobileGetDirectBankingSumery(Request $request)
                                                        {
                                                            ...
                                                            ->select('directbankings.id', 'direct_bank_customer_name', 'banks.bank_name', 'banks.id as bank_id', 'direct_bank_ref_no', 'direct_bank_amount', 'sum_id', 'directbankings.dsr_id')
                                                            ->where('pending_sum.date', '=', $request->get('date'))
                                                            ->where('pending_sum.dsr_id', '=', $request->get('dsr_id'))
        ...
        // set new amount in pending sum->sales_sum
            ...
            // Oops.. Error Occured!
                                                        ...
                                                        public function MobileEditCreditSummary(Request $request)
                                                        {
                                                            ...
                                                            $credit_update = DB::table('credits')->where('id', '=', $request->get('id'))->update(['credit_customer_name' => $request->get('customerName')]);

                                                            DB::update('update credit_items set item_id = ?, item_price = ? where id = ?', array($request->get('item_id'), $request->get('amount'), $request->get('citemid')));

                                                            $credit_sum_id = DB::table('credits')->select('id', 'sum_id')->where('id', '=', $request->get('id'))->get();
                                                            $sum_id = 0;
                                                            $credit_total = 0;
