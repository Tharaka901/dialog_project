  ...
  <div class="card lobicard lobicard-custom-control"  data-sortable="true">
   ...
   <tbody>
    ...
    <tr>
        ...
        <?php 
        if($dsr_items['status'] == 1){
            if($dsr_items['pendingSum'] == 2){
                ?>
                <td>{{ $dsr_items['qty'] }}</td>
                <?php
            }else{
                ?>
                <!--<td>{{ $dsr_items['qty'] + $dsr_items['approve_return_qty'] + $dsr_items['sale_qty'] }}</td>-->
                <td>{{ $dsr_items['qty'] + $dsr_items['approve_return_qty'] + $dsr_items['sale_qty'] - $dsr_items['retailer_qty'] }}</td>
                <?php
            }

        }else{
            ?>
            <td>{{ $dsr_items['qty'] - $dsr_items['issue_return_qty'] }}</td>
            <?php
        }
    ...
    if($dsr_items['status'] == 1){
        if($dsr_items['pendingSum'] == 2){
            $sum=$sum + ( $dsr_items['qty'] ); 
        }else{
            $sum=$sum + ( $dsr_items['qty'] + $dsr_items['approve_return_qty'] + $dsr_items['sale_qty'] - $dsr_items['retailer_qty'] ); 
        }
    }else{
        $sum=$sum+( $dsr_items['qty'] - $dsr_items['issue_return_qty'] ); 
    }
